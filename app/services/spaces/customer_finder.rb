# frozen_string_literal: true

module Spaces
  class CustomerFinder
    def self.find_or_create(space:, email:, name: nil, phone: nil, address: nil, whatsapp_opt_in: nil, consent_source: nil, locale: nil)
      raise ArgumentError, "email or phone is required" if email.blank? && phone.blank?

      name = name.to_s.strip.presence || "Guest"
      customer = space.customers.find_by("LOWER(email) = LOWER(?)", email) if email.present?
      customer ||= space.customers.find_by(phone: phone) if phone.present? && customer.nil?
      preferred_locale = LocaleResolver.normalize(locale) || space.default_locale
      customer ||= space.customers.create!(name: name, phone: phone, email: email, address: address, locale: preferred_locale)
      customer.locale = preferred_locale if preferred_locale.present? && customer.locale != preferred_locale

      if ActiveModel::Type::Boolean.new.cast(whatsapp_opt_in)
        customer.grant_whatsapp_consent(source: consent_source || "unknown")
      end

      customer.save! if customer.changed?

      customer
    end

    def self.find_existing(space:, email: nil, phone: nil)
      customer = space.customers.find_by("LOWER(email) = LOWER(?)", email) if email.present?
      customer ||= space.customers.find_by(phone: phone) if phone.present? && customer.nil?
      customer
    end
  end
end
