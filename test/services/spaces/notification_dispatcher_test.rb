...
module Spaces
  class NotificationDispatcherTest < ActiveSupport::TestCase
    ...
    test "appointment_booked sends email to space owner" do
      ...
      assert_equal [ @space.owner.email ], mail.to
      assert_includes mail.subject, @customer.name
      assert_includes mail.body.encoded, @customer.name
    end

    test "appointment_booked does nothing when owner has no email" do
